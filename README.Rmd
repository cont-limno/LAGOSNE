...
Eventually, users will be able to run `lagos_get` to supply `lagos_compile` with the flat files from LTER without having to deal with file paths. The __outputs__ of `lagos_get` and `lagos_compile` are stored in the location returned by `rappdirs::user_data_dir`.
