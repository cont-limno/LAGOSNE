...
lake_info <- function(dt, name, state){
  ...
  locus_plus_state <- suppressMessages(dplyr::left_join(dt$locus, dt$state, by = c("state_zoneid")))
  dt <- suppressMessages(dplyr::left_join(dt$lake.specific, locus_plus_state))
